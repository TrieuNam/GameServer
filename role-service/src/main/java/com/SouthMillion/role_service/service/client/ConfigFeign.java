...
public interface ConfigFeign {
    ...
    ResponseEntity<byte[]> getRoleExp(@RequestHeader(value = "If-None-Match", required = false) String ifNoneMatch);

    @GetMapping("${role.config.rolename-path-API}")
    ResponseEntity<byte[]> getRoleName(@RequestHeader(value = "If-None-Match", required = false) String ifNoneMatch);

    @GetMapping("${role.config.otherconfig-path-API}")
    ResponseEntity<byte[]> getOtherCfg(@RequestHeader(value = "If-None-Match", required = false) String ifNoneMatch);

    @GetMapping("${role.config.keyconfig-path-API}")
    ResponseEntity<byte[]> getKeyCfg(@RequestHeader(value = "If-None-Match", required = false) String ifNoneMatch);
