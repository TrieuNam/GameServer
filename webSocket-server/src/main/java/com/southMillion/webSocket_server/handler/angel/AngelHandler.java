...
public class AngelHandler implements MessageHandler {
    ...
    /** Gọi sau login: đẩy thông tin angel (2131) về client. */
    public Mono<Void> pushAll(PlayerSession session) {
        Long roleId = session.getRoleId();
        if (roleId == null) return Mono.empty();
        return Mono.fromRunnable(() -> sendAngelInfo(session, String.valueOf(roleId)));
    }
            ...
            try {
                ...
                switch (op) {
                    ...
                    case REQ_USE_APPEARANCE -> {
                        angelGrpcClient.useAppearance(userId, 0, param);
    ...
    private void sendAngelInfo(PlayerSession session, String userId) {
        GetUserAngelsResponse resp = angelGrpcClient.getUserAngels(String.valueOf(userId));
