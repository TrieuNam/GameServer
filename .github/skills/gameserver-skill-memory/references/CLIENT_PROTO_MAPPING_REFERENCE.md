...
Client anchors (đã có transport/state tối thiểu):
- MsgId registration: `document/client/LineR/assets/script/manager/MsgIdManger.ts`
- Ctrl bootstrap: `document/client/LineR/assets/script/manager/CtrlManager.ts`
- Skill ctrl/state: `document/client/LineR/assets/script/modules/skill/SkillCtrl.ts`, `SkillData.ts`
- Active skill config loader: `document/client/LineR/assets/script/config/CfgSingleSkill.ts`

Known remaining gap:
- Chưa có view/UI skill riêng trong client; hiện mới đủ tầng transport + state + config để nhận bootstrap và gọi request.
