...
- Flyway migration (skill/talent): `role-service/src/main/resources/db/migration/V2__add_role_skill_and_role_talent.sql`
...
- **Bug 4**: entity `RoleSkill`/`RoleTalent` có nhưng Flyway chưa tạo table
  - Fix: thêm migration mới thay vì sửa `V1`
- **Bug 5**: one-key level up tăng cả skill đã max level
  - Fix: giới hạn update bằng `WHERE skillLevel < MAX_SKILL_LEVEL`
...
- 2026-03-21 | Scope: role-service skill/talent | Change: bổ sung migration V2 cho `role_skill`/`role_talent` và chặn one-key upgrade vượt max | Why: hoàn tất persistence thực tế theo entity/service hiện có | Ref: `V2__add_role_skill_and_role_talent.sql`, `SkillService.java`, `RoleSkillRepository.java`
